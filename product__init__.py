...
class Product:
    ...
    def load(cls, data: dict) -> "Product":
        ...
        if not isinstance(data, dict):
            raise ValueError("Invalid product data: expected a dictionary")
        return cls(
            id=data.get('id', 0),
            name=data.get('name', ""),
...
def list_products() -> list[Product]:
    ...
    if not isinstance(products, list):
        raise ValueError("Invalid data: expected a list of products")
    return [Product.load(product) for product in products if isinstance(product, dict)]

def get_product(product_id: int) -> Product:
    """Fetches and returns a product by its ID."""
    product_data = dao.get_product(product_id)
    if not product_data or not isinstance(product_data, dict):
        raise ValueError(f"Product with ID {product_id} not found or invalid data")
